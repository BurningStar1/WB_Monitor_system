...
with st.expander("ℹ️ Формула прибыли", expanded=False):
    st.markdown(
        """
        **Операционная прибыль на артикул** рассчитывается по формуле:

        ```
        Прибыль = Реализация (ppvz_for_pay)
        ...
        **Топ-5 бренды / предметы / артикулы** — сводные рейтинги по прибыли.
        """
    )
