...
with st.expander("ℹ️ Что такое когорта и как её читать", expanded=False):
    st.markdown(
        """
        **Когорта** — группа артикулов, у которых **первая продажа**
        случилась в одном месяце. Матрица показывает, сколько продаж
        (или выручки) принесла когорта на 1-м, 2-м, 3-м… месяцах жизни.

        **Два режима просмотра:**

        - **Retention (% от 1-го месяца)** — сколько процентов объёма 1-го месяца
          держится в последующих. **>100%** = когорта выросла, **<100%** = сжалась.
          Хороший товар держит ≥50% во 2-м месяце.

        - **Абсолютные значения** — реальные числа в штуках/рублях по месяцам.
          Удобно для оценки «размера» когорт между собой.

        **Интерпретация:**
        - *Сильные когорты сверху* → новые артикулы хорошо заходят
        - *Быстрое затухание (retention < 30% на 2-м мес.)* → одноразовые хиты или сезонка
        - *Долгоиграющие когорты* → ядро ассортимента, фокус на поддержание остатков
        """
    )
