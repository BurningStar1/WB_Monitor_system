...
with st.expander("ℹ️ Методика когортного анализа (подробно)", expanded=False):
    st.markdown(
        """
        ### Что такое когорта

        **Когорта** — группа артикулов, объединённых по одному признаку:
        у всех была *первая продажа* в одном и том же календарном месяце.
        Если артикул впервые продался в марте 2026 — он живёт в когорте
        «2026-03» навсегда, даже если продаётся до сих пор.

        Зачем это нужно: обычные сравнения «месяц к месяцу» мешают друг
        с другом новые и старые артикулы. Когорты отделяют их друг от
        друга и показывают, как ведёт себя *каждое поколение* ассортимента
        по мере взросления.

        ### Как строится матрица

        Ось **Y** — когорта (месяц первой продажи).
        Ось **X** — возраст когорты: `M+0` — месяц рождения, `M+1` —
        второй месяц, `M+2` — третий и т.д.
        В ячейке на пересечении — *суммарный* объём продаж (штук или
        рублей) этой когорты за этот месяц её жизни.

        **Пример.** Когорта «2026-01» в ячейке `M+2` = сумма продаж всех
        артикулов, впервые проданных в январе 2026, *в марте 2026*.

        ### Два режима просмотра

        | Режим | Что показывает | Как читать |
        |---|---|---|
        | **Retention (% от M+0)** | Доля от объёма первого месяца | 100% → столько же, сколько в `M+0`; >100% → когорта выросла; <50% во 2-м мес. → быстрое затухание |
        | **Абсолютные значения** | Реальные штуки или рубли | Сравнение «размера» когорт между собой |

        ### Метрики

        - **Количество (шт)** — физический объём продаж из
          `mart.sales_daily.sales_count`.
        - **Выручка (₽)** — `net_revenue` из той же витрины.

        Для финансовой когорты ориентируемся на *выручку* (рубли важнее
        штук), для ассортиментной — на *штуки*.

        ### Типовые паттерны

        | Паттерн | Что значит | Действие |
        |---|---|---|
        | Тёмная диагональ сверху | Старые когорты долгожители | Поддерживать остатки, не прерывать |
        | Светлые правые столбцы | Старые когорты затухают | Обновлять ассортимент |
        | Сильные свежие когорты | Новинки заходят | Наращивать закупку новых артикулов |
        | Retention `M+1` > 100% | Когорта растёт после запуска | Отличный сигнал — товар «раскручивается» |
        | Retention `M+1` < 30% | Одноразовые хиты / сезонка | Проверить зависимость от акций |

        ### Ограничения метода

        - **Не учитывает размер когорты** — 1 артикул и 100 артикулов
          в одной когорте будут иметь одинаковый retention, но разный
          абсолютный объём. Проверяйте таблицу *Размер когорт* внизу.
        - **Чувствителен к выбросам** — один мощный артикул может
          «вытащить» всю когорту; смотрите медиану отдельно, если
          требуется.
        - **Нужна глубина истории** — для ретеншн-матрицы ≥6 месяцев,
          для уверенных выводов — 12+.

        ### Когда применять
        - Оценить, как «стареют» поколения ассортимента
        - Понять, устойчив ли бизнес — когорты должны быть стабильны
          в ретеншне 30-50%+ на `M+3`
        - Проверить эффективность запусков новых карточек
        """
    )
