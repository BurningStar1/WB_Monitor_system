...
with st.expander("ℹ️ Что такое CM1 / CM2 / CM3", expanded=False):
    st.markdown(
        ...
        **Contribution Margin** — маржинальная прибыль на один проданный товар,
        ...
        - **CM1** = Выручка за ед. − Себестоимость
          *(валовая маржа по товару — показывает наценку)*
        - **CM2** = CM1 − Услуги WB (логистика + хранение + штрафы + эквайринг + приёмка)
          *(маржа после площадки — показывает «что осталось» после WB)*
        - **CM3** = CM2 − Налоги
          *(чистая маржа — деньги в кармане)*

        **Как использовать:**
        - **CM3 < 0** → убыточный артикул, срочно разобраться с ценой/затратами
        - **CM1 < 0** → продаём ниже себестоимости (часто — акции, демпинг)
        - **CM2 ≈ CM1** → очень эффективная логистика (крупные лёгкие товары)

        Расчёт делается на *единицу* — делим все суммы на число продаж артикула.
        """
    )
