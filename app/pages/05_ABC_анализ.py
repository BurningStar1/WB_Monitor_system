...
with st.expander("ℹ️ Методика ABC + XYZ", expanded=False):
    st.markdown(
        """
        ### ABC-анализ (правило Парето)

        Артикулы ранжируются по убыванию выбранной метрики. Каждому присваивается
        буква A/B/C в зависимости от накопленной доли:

        | Группа | Накопленный вклад | Смысл                 |
        |:------:|:-----------------:|:----------------------|
        |   A    | 0 — 80 %          | Критичные для бизнеса |
        |   B    | 80 — 95 %         | Средние               |
        |   C    | 95 — 100 %        | «Длинный хвост»       |

        Границы можно менять сверху (например, 70/90 делают классификацию строже).

        **Трёхмерный ABC** — классификация одновременно по трём метрикам:
        Выручка × Прибыль × Продажи. Код вида **AAB** значит: A по выручке,
        A по прибыли, B по количеству продаж. Это помогает увидеть артикулы,
        которые «тянут оборот, но не прибыль».

        ### XYZ-анализ (стабильность спроса)

        Для каждого артикула считается коэффициент вариации (CV) ежедневных
        заказов: `std(заказов) / mean(заказов) × 100%`.

        | Группа | CV              | Характер спроса |
        |:------:|:---------------:|:----------------|
        |   X    | ≤ 10 %          | Стабильный      |
        |   Y    | 10 — 25 %       | Переменный      |
        |   Z    | > 25 %          | Эпизодический   |

        Если у артикула меньше 4 дней продаж — ставится прочерк («—»).

        ### Матрица ABC × XYZ — рекомендации

        | Клетка | Что это           | Решение                                  |
        |:------:|:------------------|:-----------------------------------------|
        | **AX** | Топ-стабильные    | Автозаказ, страховой запас, держать в топе |
        | **AY** | Топ-переменные    | Точный прогноз, следить за сезонностью   |
        | **AZ** | Топ-непредсказуемые | Ручной контроль, не допустить OOS      |
        | **BX** | Середняки стабильные | Автоматизация, экономия на закупке    |
        | **BY** | Середняки переменные | Стандартное управление                |
        | **BZ** | Середняки нестабильные | Уменьшить запас, проверить маржу    |
        | **CX** | Хвост стабильный  | Минимальный запас                        |
        | **CY** | Хвост переменный  | Под заказ / снять с ассортимента         |
        | **CZ** | Хвост непредсказуемый | Снять, перепозиционировать            |

        ### Когда применять

        - Перед закупкой сезона — чтобы понять, куда вложить бюджет
        - При ассортиментной чистке — какие SKU убрать без потери выручки
        - При распределении маркетинговых бюджетов — рекламу лить на **AX/AY**
        - При установке страхового запаса — больше на X, меньше на Z
        """
    )
