...
with st.expander("ℹ️ Какие события отслеживаем", expanded=False):
    st.markdown(
        """
        **3 блока критичных событий:**

        1. **📉 Низкий запас** — артикулы, у которых закончится товар раньше
           порога (по умолчанию 7 дней). Считается как
           `остаток / средний_заказ_в_день`. Нужно ставить поставку.

        2. **💸 Убыточные артикулы** — прибыль < 0 за окно анализа.
           Порог убытка в ₽ — минимальная сумма убытка для включения
           (по умолчанию 0 — любой убыток). Требует пересмотра цены или закупочной.

        3. **↘️ Падение заказов WoW** — артикулы с падением заказов
           текущие 7 дней vs предыдущие 7 дней больше порога (по умолчанию 30%).
           Учитываются только артикулы с базой ≥3 заказа, чтобы не шуметь по мелочам.

        **Пороги настраиваются сверху** — снижайте чтобы увидеть больше,
        повышайте чтобы видеть только самые критичные.
        """
    )
