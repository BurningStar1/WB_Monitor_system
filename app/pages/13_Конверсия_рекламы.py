...
with st.expander("ℹ️ Как оценивается эффективность рекламы", expanded=False):
    st.markdown(
        """
        **Ключевые метрики:**
        - **CTR (Click-Through Rate)** = Клики / Показы × 100%
          — насколько карточка цепляет аудиторию в выдаче.
          *Норма WB:* 3–8%, <2% — плохая карточка.
        - **CPC (Cost Per Click)** = Расход / Клики
          — сколько платим за один переход.
        - **CR (Conversion Rate)** = Заказы из рекламы / Клики × 100%
          — эффективность воронки. *Норма:* 5–15%.
        - **CPO (Cost Per Order)** = Расход / Заказы из рекламы
          — стоимость одного заказа.
        - **ROI (Return On Investment)** = (Выручка − Расход) / Расход × 100%
          — окупаемость рекламы. *Норма:* >100%.
        - **ДРР (Доля Рекламных Расходов)** = Расход / Выручка от рекламы × 100%
          — обратная ROI метрика. *Норма:* <30%.

        **Интерпретация:**
        - **ROI > 200%** → масштабируйте кампанию, увеличивайте ставку
        - **ROI 100–200%** → стабильный профит, держите уровень
        - **ROI < 100%** → реклама убыточна, пересмотрите ставки/таргет/карточку
        """
    )
