...
with st.expander("ℹ️ О сравнении недель", expanded=False):
    st.markdown(
        """
        Сравниваем **текущую ISO-неделю** с **предыдущей** по ключевым метрикам.

        **Δ%** = `(Текущая − Предыдущая) / Предыдущая × 100`

        **Цвета стрелок:**
        - 🟢 Зелёный (↑) — рост (для выручки/прибыли/продаж) или снижение (для возвратов/расходов)
        - 🔴 Красный (↓) — обратная динамика

        **Таблица артикулов внизу** показывает топ-драйверы изменений:
        какие конкретно товары выросли или упали сильнее всего.

        **Совет:** обращайте внимание на артикулы с крупным **Δ выручки в ₽**,
        а не только на процентные изменения — артикул с +500% может быть менее важен
        чем флагман с −5%.
        """
    )
